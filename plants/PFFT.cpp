...
PFFT::PFFT(int N) : N(N), wN(0), _wN(0), reverse(0), side(0) {
  vals[0] = vals[1] = 0;
...
Complex PFFT::makeWn(double n, double N){
...
unsigned int PFFT::reverseBits(unsigned int n){
  unsigned int res = 0;
  for(int i = 0; i < logN; i++) {
    res <<= 1;
    res += (n&1);
...
void PFFT::transform(Complex* input, Complex* output, int stride, int offset, bool inverse){
